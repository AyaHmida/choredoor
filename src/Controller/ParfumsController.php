...
class ParfumsController extends AbstractController
{
    ...
    #[Route('/parfums/update/{id}', name: 'parfums_update')]
public function edit(Request $request, EntityManagerInterface $entityManager, Parfums $parfums): Response
{
    $form = $this->createForm(ParfumsType::class, $parfums);
    $form->handleRequest($request);
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        ...
        $this->addFlash('success', 'The parfums has been updated successfully.');

        return $this->redirectToRoute('app_parfums_list', ['id' => $parfums->getId()]);
    }

    return $this->render('parfums/update.html.twig', [
